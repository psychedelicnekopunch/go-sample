...
func (r *RegexpSample) Email() *regexp.Regexp {
	...
	// reg := `(?i)^[0-9a-z-_]+[0-9a-z-._]*[0-9a-z-_]+@(linshiyouxiang)[0-9a-z-.]+[0-9a-z]+$`
	return regexp.MustCompile(reg)
}
...
func (r *RegexpSample) SomeName() *regexp.Regexp {
	reg := `(?i)(compute|ddos-guard|dehost|googleusercontent)`
	return regexp.MustCompile(reg)
}
...
func main() {
	...
	// str := "ec2.ap-northeast-1.compute.amazonaws.com"
	// regSomeName := reg.SomeName()
	// fmt.Print(regSomeName.MatchString(str), "\n")// true
}
